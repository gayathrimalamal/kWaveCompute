%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% --kWave Script for Synthetic Transmit Aperture Transmission (L11-5v transducer)
% --Single Element Transmit, All Elements Receive
...
probe_geometry = linspace(-Trans.tx_width/2, Trans.tx_width/2, Trans.tx_N_elements)'; % L11-5V probe geometry

Trans.tx_active_SA = 1: Trans.tx_N_elements;  % Number of active transducer elements at transmit

%% DEFINE THE K-WAVE GRID

endDepth = 0.7*2e-2;     % Maximum depth in diagonal dir [m](1/sqrt(2) = 0.7)
...
figure,imagesc(lat_vec.*1000, ax_vec.*1000,medium.sound_speed); % Plotting the speed of sound map
title('SoS map');
colorbar;
figure,imagesc(lat_vec.*1000, ax_vec.*1000,medium.density); % Plotting the density map
title('Density map');
colorbar;
...
% Create empty kWaveArray for sensor
karraySens = kWaveArray('BLITolerance', 0.05, 'UpsamplingRate', 10);

% Creating each transducer element by combining grid points
for Nc = 1: Trans.tx_N_elements
    karraySens.addRectElement([kgrid.x_vec(1), probe_geometry(Nc, 1)],Trans.tx_kerf, Trans.tx_element_width, rotation);
end

% Defining sensor array
karraySens.setArrayPosition([0, 0], 0);
sensor.mask = zeros(Nz, Nx);
sensor.mask = karraySens.getArrayBinaryMask(kgrid);
...
acq = 'STA';
for Nc = 1:length(Trans.tx_active_SA)
    
    Nc_TxC = Trans.tx_active_SA(Nc);
    
    % Create empty kWaveArray for source
    karraySrc = kWaveArray('BLITolerance', 0.05, 'UpsamplingRate', 10);
    
    % Creating a transducer element by combining grid points
    karraySrc.addRectElement([kgrid.x_vec(1), probe_geometry(Trans.tx_active_SA(Nc_TxC), 1)],Trans.tx_kerf, Trans.tx_element_width, rotation);
    karraySrc.setArrayPosition([0, 0], 0);
    
    % Defining source array
    source.p_mask = zeros(Nz, Nx);
    source.p_mask = karraySrc.getArrayBinaryMask(kgrid);
    
    % Note that the source strength is not scaled by acoustic impedance as a pressure source is used (Z=P/V)
    source_sig = source_strength*toneBurst(1/kgrid.dt, source_freq, source_cycles);
    source.p = karraySrc.getDistributedSourceSignal(kgrid, source_sig);
    
    if(GPU)
        DATA_CAST       = 'gpuArray-single';
        input_args = {'PMLSize', [PML_X_size, PML_Z_size], 'PlotPML', false, ...
    ...
    sensor_data(:,:,Nc) = karraySens.combineSensorData(kgrid, sensor_data_grid_points); % Combine the grid points to get in terms of physical transducer elements
end
...
if(DO_BEAMFORMING)
    ...
    for txIdx = 1: length(dataset.Trans.tx_active_SA)
        TxC = dataset.probe_geometry(txIdx); % Transmit center for STA
        beamformedDataSTA (:,:,txIdx) = DAS_RF(acq,squeeze(rawData(:, :, txIdx)), timeVector, x_grid, z_grid, dataset.probe_geometry, TxC, dataset.Trans.tx_c0*ones(size(x_grid)), Fnum);
    end
    
    beamformedDataDAS = sum(beamformedDataSTA(:,:,:), 3);
